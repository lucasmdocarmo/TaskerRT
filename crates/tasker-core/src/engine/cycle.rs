...
use crate::domain::{Arena, Job, JobId, JobState, PriorityClass, TransitionError, VirtualTime};
use crate::policy::{
    BackfillScratch, DependencyError, DependencyTracker, DispatchDecision, Disposition, OrderKey,
    PackBudget, PriorityConfig, Readiness, ReadyEntry, ReadySet, RunningJob, easy_backfill, pack,
    score,
};
...
impl Scheduler {
    ...
    pub fn on_completed(
        ...
        jobs.get_mut(id)
            .ok_or(LifecycleError::UnknownJob(id))?
            .try_transition(JobState::Completed)?;
        promoted.clear();
    ...
    pub fn on_failed(
        ...
        jobs.get_mut(id)
            .ok_or(LifecycleError::UnknownJob(id))?
            .try_transition(JobState::Failed)?;
        cancelled.clear();
